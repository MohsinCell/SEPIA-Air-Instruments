...
export const Stage = forwardRef<HTMLDivElement, StageProps>(function Stage({ instrument, videoRef, canvasRef }, ref) {
    const IconComponent = getInstrumentIcon(instrument.icon);
    return (<main className="stage" ref={ref}>
        
        <video ref={videoRef} className="stage__video" playsInline muted autoPlay/>

        
        <canvas ref={canvasRef} className="stage__canvas" width={VIDEO_CONFIG.width} height={VIDEO_CONFIG.height}/>
